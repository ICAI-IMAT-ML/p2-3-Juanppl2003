...
class LinearRegressor:
    ...
    def fit_multiple(self, X, y):
        ...
        betas = np.linalg.inv(X_bias.T @ X_bias) @ X_bias.T @ y
    ...
    def predict(self, X):
        ...
        if np.ndim(X) == 1 or X.shape[1] <2:
            ...
            predictions = X_bias @ np.concatenate([[self.intercept], self.coefficients])
...
def evaluate_regression(y_true, y_pred):
    ...
    #El R² mide qué tan bien se ajustan las predicciones del modelo a los datos reales.
    ...
    # TODO: Calculate RMSE
    #raiz cuadrada de la media de los errores al cuadrado
